# Phase 5-23: PlayerSystem class/status helper 분리

## 목적

플레이어 state/result payload에서 쓰이는 class label, player status effect 계산을 `PlayerSystem` 경계로 분리했다.
...
- `src/server/systems/PlayerSystem.ts` 확장
  - class label/status TypeScript 계약 작성
- `server.js` 변경
  - class label/status 보조 계산만 PlayerSystem 경유로 전환
