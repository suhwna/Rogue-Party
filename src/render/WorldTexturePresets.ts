export interface TileTheme {
  ...
  brick: readonly string[];
  seam: string;
  accentA: string;
  accentB: string;
}

export const chapterTileThemes: Record<1 | 2 | 3, TileTheme> = {
  1: {
...
function safeChapter(chapter: number): 1 | 2 | 3 {
  const value = Math.max(1, Math.min(3, Math.floor(Number(chapter) || 1)));
...
function drawTileSeams(ctx: CanvasRenderingContext2D, variant: number, brickColor: string, seamColor: string): void {
...
function drawTileBorder(ctx: CanvasRenderingContext2D): void {
...
export function drawFloorTile(ctx: CanvasRenderingContext2D, chapter: number, variantValue: number): void {
  const chapterId = safeChapter(chapter);
  const variant = variantIndex(variantValue);
  const theme = chapterTileThemes[chapterId];
  ctx.fillStyle = theme.bases[variant] ?? theme.bases[0] ?? "#211817";
  ctx.fillRect(0, 0, 64, 64);
  drawTileSeams(ctx, variant, theme.brick[variant % theme.brick.length] ?? theme.brick[0] ?? "#33231f", theme.seam);
...
export function drawLegacyFloorTile(ctx: CanvasRenderingContext2D, variantValue: number): void {
  const variant = variantIndex(variantValue);
...
export function drawDefaultWallBlock(ctx: CanvasRenderingContext2D): void {
...
export function drawWallBlock(ctx: CanvasRenderingContext2D, chapter: number): void {
  const chapterId = safeChapter(chapter);
  const main = chapterId === 1 ? "#27201b" : chapterId === 2 ? "#17291c" : "#1b1730";
  const brick = chapterId === 1 ? "#3a2d23" : chapterId === 2 ? "#25452b" : "#302450";
  const shine = chapterId === 1 ? "rgba(248,243,233,0.14)" : chapterId === 2 ? "rgba(190,242,100,0.14)" : "rgba(245,208,254,0.15)";
...
export function drawDefaultTorch(ctx: CanvasRenderingContext2D): void {
...
export function drawTorch(ctx: CanvasRenderingContext2D, chapter: number): void {
  const chapterId = safeChapter(chapter);
  const flame = chapterId === 1 ? "#f97316" : chapterId === 2 ? "#84cc16" : "#8b5cf6";
  const core = chapterId === 1 ? "#facc15" : chapterId === 2 ? "#d9f99d" : "#dbeafe";
  const ember = chapterId === 1 ? "#ef4444" : chapterId === 2 ? "#22c55e" : "#60a5fa";
  const wood = chapterId === 2 ? "#25452b" : chapterId === 3 ? "#302450" : "#5b3a22";
  px(ctx, 14, 18, 4, 24, wood);
  px(ctx, 11, 14, 10, 7, chapterId === 1 ? "#6b4a2b" : chapterId === 2 ? "#1f3f2b" : "#21142f");
