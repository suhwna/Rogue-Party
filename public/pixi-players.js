(() => {
  ...
  function renderPlayer(renderer, player, now, visuals, selfId) {
    ...
    if (player.shield > 0) renderer.ring(pos.x, pos.y, 33 * scaleBase, "#bfdbfe", 0.5, 3);
    if (player.statusEffects?.includes("taunt_guard")) renderer.ring(pos.x, pos.y, 42 * scaleBase, "#f97316", 0.34, 4);
    if (player.id === selfId) {
      renderer.bar(pos.x, pos.y - 56 * scaleBase, 86, 8, player.hp / player.maxHp, "#ef4444");
      if (player.shield > 0) renderer.bar(pos.x, pos.y - 46 * scaleBase, 86, 4, player.shield / Math.max(1, player.maxHp * 0.45), "#93c5fd");
    }
  }
